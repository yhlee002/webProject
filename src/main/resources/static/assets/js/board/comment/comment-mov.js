$(document).ready(function () {
                ...
                if (data.totalCommentCnt == 0) {
                    ...
                    for (let i = 0; i < data.list.length; i++) {
                        ...
                        str +=
                            ...
                            "<td><button class='upBtn' type='button' onclick=''>추천</button></td>" +
                            "<td></td>" +
                            "</tr>";
                    }
                    ...
                    for (let i = 0; i < result.length; i++) {
                        for (let j = 0; j < commIdArr.length; j++) {
                            if (commIdArr[j].value == result[i].writerId) {
                                let td = commIdArr[j].parentNode.parentNode.children[6];
                                td.innerHTML = "<button class='modifyBtn' type='button'>수정</button><button class='deleteBtn' type='button'>삭제</button>";
                            }
                        }
                    }
                }
    ...
    $('.upBtn').on("click", function () {
        ...
        let commId = $('.upBtn').prev().val(); // input[hidden=hidden]의 값(comment_id)
        console.log(commId);

        //DB에 해당 아이디와 로그인되어있는 회원의 정보가 매칭되는 엔티티가 있는지 확인하기 위해
        $.ajax({
            url: "/movieInfo/comment/recommended/1",
            type: "post",
            data: {
                "commentId": commId,
                "memNo": memNo
            ...
            beforeSend: function (xhr) {
                ...
                console.log("upBtn 클릭 이벤트 ajax 결과\n" + data)
                if (data != null) { // 좋아요 -1
                    $(this).text("up");
                } else { // 좋아요 +1
                    $(this).text("취소");
